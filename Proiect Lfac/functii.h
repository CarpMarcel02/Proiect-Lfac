...
int cl_Declarare_definitie(char nume[]) // cl_decl_def
{
...
int f_Declarare_definitie(char nume[], char tip[]) // f_decl_def
{
    num_func++;
...
int v_Declarare_definitie(char nume[], char tip, int dimensiune) // v_decl
{
...
void variabila_bool_declarare_init(char nume[], char valoare_bool[]) // bool_decl
{
    ...
    {
        variabile[num_var].bool_valoare = 1;
    }
    else if (strcmp(valoare_bool, "false") == 0)
    {
        variabile[num_var].bool_valoare = 0;
    }
    variabile[num_var].init = 1;
    num_var++;
}
...
void va_bool_atribuire(char nume[], int valoare)
{
    ...
    variabile[declarat].bool_valoare = valoare;
    variabile[declarat].init = 1;
}
                 ...
                 if(strcmp(variabile[i].tip,TipAsignareId)==0 && OkAsignareId==1 ) ////////////////////////////////
                    {
                        if(strcmp(variabile[i].tip,"bool")==0)
                            {
                                variabile[i].bool_valoare=adevarat;
                                adevarat=-1;
                                return 1;
                                }
                        variabile[i].int_valoare=inlocuireInt;
                 ...
                 if(strcmp(variabile[i].tip,"bool")==0 && TipBoolGlobal==1 )
                   {
                    ...
                    if(adevarat==1)
                        {
                            variabile[i].bool_valoare=true;
                            return 1;
                        }
                    else if(adevarat==0)
                    {
                        variabile[i].bool_valoare=false;
                        return 1;
                    }
                     TipBoolGlobal=0;
                     adevarat=-1;
